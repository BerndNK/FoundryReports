...
namespace FoundryReports.Core.Source
{
    public class JsonDataSource : JsonSourceBase, IDataSource
    {
        ...
        public async Task LoadCustomer()
        {
            ...
            {
                var toIgnore = singleCustomer.MonthlyProductReportsList.Where(x => x.IsPredicted).ToList();
                foreach (var monthlyProductReport in toIgnore)
                {
                    singleCustomer.MonthlyProductReportsList.Remove(monthlyProductReport);
                }
                ...
                foreach (var monthlyProductReport in singleCustomer.MonthlyProductReportsList)
                {
                    var correspondingProduct =
                        _products.FirstOrDefault(m => m.Name == monthlyProductReport.ProductName);
